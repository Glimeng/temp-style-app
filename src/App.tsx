...
import { ArrowRight, Bell, Check, ChevronRight, CloudRain, CloudSun, Compass, Heart, Home, LogOut, MapPin, Plus, RefreshCw, Search, Shirt, Sparkles, Sun, Thermometer, Trash2, UserRound, Wind, X } from 'lucide-react'
import { createWardrobeItem, deleteWardrobeItem, demoLogin, getOutfits, getWardrobe, getWeather, markOutfitWorn, saveOutfitFeedback, updateWardrobeItem } from './api'
import './App.css'
...
type City = { name: string; initial: string; temp: string; weather: string; summary: string; slots: [string, string, string]; apparentTemp?: string; humidity?: number; wind?: string; isLive?: boolean }
...
export default function App() {
  ...
  const refreshWeather = useCallback((targetCity: City, coordinates?: { latitude: number; longitude: number }) => {
    setWeatherLoading(true)
    return getWeather(coordinates ? '当前位置' : targetCity.name, coordinates).then((data) => {
      setCity(data)
      setTemp(data.temp)
      setFeltTemp(`体感 ${data.apparentTemp}°`)
      return data
    }).finally(() => setWeatherLoading(false))
  }, [])
  useEffect(() => {
    void Promise.resolve().then(() => refreshWeather(cityOptions[0]))
    if (!window.isSecureContext || !navigator.geolocation) return
    navigator.geolocation.getCurrentPosition(
      (position) => { void refreshWeather(cityOptions[0], { latitude: position.coords.latitude, longitude: position.coords.longitude }) },
      () => undefined,
  ...
  const filteredItems = useMemo(() => wardrobeItems
    .filter((item) =>
      (category === '全部' || item.category === category) &&
      (!wardrobeSearch.trim() || `${item.name}${item.tags.join('')}${item.color}`.toLowerCase().includes(wardrobeSearch.trim().toLowerCase())) &&
      (filters.status === '全部' || item.status === filters.status) &&
      (filters.recognition === '全部' || item.recognition === filters.recognition) &&
      (!filters.tag.length || filters.tag.some((tag) => item.tags.includes(tag)))
    )
    .sort((a, b) =>
      (a.status === '暂时不能穿' ? 1 : 0) - (b.status === '暂时不能穿' ? 1 : 0) || a.id - b.id
    ), [category, filters, wardrobeItems, wardrobeSearch])
  ...
  const toggleItemStatus = (itemId: number) => setWardrobeItems((current) => current.map((item) => { if (item.id !== itemId) return item; const nextStatus = item.status === '可穿' ? '暂时不能穿' : '可穿'; const token = apiToken(); if (token) void updateWardrobeItem(token, itemId, { status: nextStatus }).catch(() => undefined); setNotice(nextStatus === '可穿' ? `${item.name} 已恢复可穿` : `${item.name} 已标记为暂时不能穿`); return { ...item, status: nextStatus } }))
  const saveEditedItem = (nextItem: Item) => { const token = apiToken(); if (token) void updateWardrobeItem(token, nextItem.id, nextItem).catch(() => undefined); setWardrobeItems((current) => current.map((item) => item.id === nextItem.id ? nextItem : item)); setEditingItem(null); setNotice(`${nextItem.name} 的衣物信息已更新`) }
  const removeWardrobeItem = (itemId: number) => {
    const item = wardrobeItems.find((entry) => entry.id === itemId)
    const remainingOutfits = outfits.filter((outfit) => !outfit.items.includes(itemId))
    const token = apiToken()
    if (token) void deleteWardrobeItem(token, itemId).catch(() => undefined)
    setWardrobeItems((current) => current.filter((entry) => entry.id !== itemId))
    setOutfits(remainingOutfits)
    if (selectedOutfit.items.includes(itemId) && remainingOutfits[0]) setSelectedOutfit(remainingOutfits[0])
    setNotice(`${item?.name || '该衣物'} 已从衣橱删除`)
  }
  ...
  const logout = () => {
    ...
    setSelectedOutfit(baseOutfits[0])
    setPreferredStyles(['通勤', '简约', '日系'])
    setTab('home')
    setNotice('已退出当前账号，可继续匿名浏览')
  }
  const switchAccount = () => {
    window.localStorage.removeItem('zhiwen-demo-token')
    setIsLoggedIn(false)
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark"><Thermometer size={17} /></div><span>知温</span></div><div className="top-actions"><button className="icon-btn" aria-label="通知"><Bell size={18} /></button><button className="avatar-btn" aria-label={isLoggedIn ? '账户管理' : '登录'} onClick={() => isLoggedIn ? setShowAccountSheet(true) : setShowLogin(true)}>林</button></div></header>
    <main className="page-content">{showCityPicker ? <CityPicker city={city} history={cityHistory} onBack={() => setShowCityPicker(false)} onSelect={(nextCity) => { setCity(nextCity); setCityHistory((history) => [nextCity, ...history.filter((entry) => entry.name !== nextCity.name)].slice(0, 4)); setShowCityPicker(false); setNotice(`已切换至${nextCity.name}，正在更新实时天气`); void refreshWeather(nextCity) }} /> : <>{tab === 'home' && <HomePage city={city} temp={temp} feltTemp={feltTemp} setFeltTemp={setFeltTemp} weatherLoading={weatherLoading} onRefreshWeather={() => { void refreshWeather(city) }} explore={explore} setExplore={setExplore} activeOutfits={rankedOutfits} openOutfit={openOutfit} wearOutfit={wearOutfit} setShowCityPicker={setShowCityPicker} setShowLogin={setShowLogin} showGuide={showGuide} setShowGuide={setShowGuide} />}{tab === 'wardrobe' && <WardrobePage category={category} setCategory={setCategory} filteredItems={filteredItems} search={wardrobeSearch} setSearch={setWardrobeSearch} filters={filters} setFilters={setFilters} tags={availableTags} showFilters={showFilters} setShowFilters={setShowFilters} onAddClothing={addClothing} onToggleStatus={toggleItemStatus} onEdit={setEditingItem} onDelete={removeWardrobeItem} />}{tab === 'explore' && <ExplorePage outfits={activeOutfits} preferredStyles={recommendationStyles} openOutfit={openOutfit} />}{tab === 'detail' && <DetailPage outfit={selectedOutfit} items={wardrobeItems} temp={temp} worn={worn} setWorn={setWorn} feedback={feedback} saveFeedback={saveFeedback} replaceOutfit={replaceOutfit} openOutfit={openOutfit} outfits={activeOutfits} onReplaceItem={beginReplaceItem} onBack={() => setTab(returnTab)} />}{tab === 'replace' && <ReplaceItemPage outfit={selectedOutfit} items={wardrobeItems} replaceItemId={replaceItemId} candidates={replacementCandidates} onBack={() => { setReplaceItemId(null); setTab('detail') }} onSelect={applyReplacement} />}{tab === 'profile' && <ProfilePage explore={explore} setExplore={setExplore} preferredStyles={preferredStyles} isLoggedIn={isLoggedIn} setShowLogin={setShowLogin} setNotice={setNotice} onOpenEditor={() => setShowProfileEditor(true)} />}</>}</main>
    ...
    {editingItem && <ItemEditSheet item={editingItem} onClose={() => setEditingItem(null)} onSave={saveEditedItem} />}
    {showAccountSheet && <AccountSheet onClose={() => setShowAccountSheet(false)} onSwitch={switchAccount} onLogout={logout} />}
...
function HomePage({ city, temp, feltTemp, setFeltTemp, weatherLoading, onRefreshWeather, explore, setExplore, activeOutfits, openOutfit, wearOutfit, setShowCityPicker, setShowLogin, showGuide, setShowGuide }: { city: City; temp: string; feltTemp: string; setFeltTemp: (v: string) => void; weatherLoading: boolean; onRefreshWeather: () => void; explore: boolean; setExplore: (v: boolean) => void; activeOutfits: Outfit[]; openOutfit: (outfit: Outfit) => void; wearOutfit: (outfit: Outfit) => void; setShowCityPicker: (v: boolean) => void; setShowLogin: (v: boolean) => void; showGuide: boolean; setShowGuide: (v: boolean) => void }) {
  return <>
    ...
    <section className="weather-panel"><div className="weather-head"><div className="location"><MapPin size={16} />{city.name} <span>· {city.isLive ? '实时天气' : '天气暂不可用'}</span></div><button className="text-btn" onClick={() => setShowCityPicker(true)}>切换城市 <ChevronRight size={14} /></button></div><div className="weather-main"><div><div className="temp-display">{temp}<span>°</span></div><div className="weather-desc"><CloudSun size={17} /> {city.weather} <strong>体感 {city.apparentTemp ?? temp}°</strong></div></div><div className="weather-note"><Wind size={17} /><span>{city.wind ?? '微风'}<br /><b>湿度 {city.humidity ?? 50}%</b></span></div></div><div className="day-slots"><WeatherSlot time="早晨 07:30" temp={city.slots[0]} icon={<CloudSun size={18} />} note={Number(city.slots[0].replace('°', '')) < 15 ? '微凉' : '舒适'} /><WeatherSlot time="白天 12:00" temp={city.slots[1]} icon={<Sun size={18} />} note={Number(city.slots[1].replace('°', '')) > 28 ? '偏热' : '舒适'} active /><WeatherSlot time="晚间 18:00" temp={city.slots[2]} icon={city.weather.includes('雨') ? <CloudRain size={18} /> : <CloudSun size={18} />} note={city.weather.includes('雨') ? '注意雨具' : '舒适'} /></div><div className="temperature-input"><div className="input-title"><Thermometer size={16} /> 你的体感温度</div><div className="temp-control"><input aria-label="体感温度" value={feltTemp} onChange={(e) => setFeltTemp(e.target.value)} /><span>用于校准今天的推荐</span><button className="small-action" aria-label="刷新实时天气" title="刷新实时天气" onClick={onRefreshWeather} disabled={weatherLoading}><RefreshCw size={14} className={weatherLoading ? 'spin' : ''} /></button></div></div></section>
...
function ClothingCard({ item, onStatus, onEdit, onDelete }: { item: Item; onStatus: () => void; onEdit: () => void; onDelete: () => void }) {
  const [menuOpen, setMenuOpen] = useState(false)
  const closeAndRun = (action: () => void) => { setMenuOpen(false); action() }
  return <div className={`clothing-card ${item.status !== '可穿' ? 'unavailable' : ''}`}><div className="clothing-card-top"><ClothingVisual item={item} /><button className="more-btn" aria-label={`${item.name}更多操作`} aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}><span /><span /><span /></button>{menuOpen && <div className="clothing-menu"><button onClick={() => closeAndRun(onEdit)}>编辑</button><button onClick={() => closeAndRun(onStatus)}>{item.status === '可穿' ? '暂不可穿' : '可穿'}</button><button className="delete-action" onClick={() => closeAndRun(onDelete)}>删除</button></div>}</div><div className="clothing-card-info"><div><h3>{item.name}</h3><p>{item.category} · {item.color}</p></div><span className={`status-chip ${item.status !== '可穿' ? 'paused' : ''}`}>{item.status === '可穿' ? <Check size={12} /> : <span className="pause-icon"><i /><i /></span>} {item.status}</span></div><div className="tag-list">{item.tags.slice(0, 2).map((tag) => <span key={tag}>{tag}</span>)}</div></div>
}
...
const styleGroups: { title: string; options: string[] }[] = [
  { title: '日常场景', options: ['通勤', '休闲', '约会', '运动', '旅行'] },
  { title: '风格气质', options: ['简约', '极简', '轻熟', '复古', '学院', '甜酷', '街头', '文艺', '清冷'] },
  { title: '地域风格', options: ['韩系', '日系', '美式', '欧美', '港风', '国风', '法式'] },
  { title: '潮流元素', options: ['Y2K', '多巴胺', '老钱风', '机能风', '山系', 'KPOP', '洛丽塔'] },
]
const presetStyles = new Set(styleGroups.flatMap((group) => group.options))
...
function ProfileEditSheet({ title = '编辑风格偏好', styles, onClose, onSave }: { title?: string; styles: string[]; onClose: () => void; onSave: (styles: string[]) => void }) {
  const [draft, setDraft] = useState<string[]>(styles)
  const [customInput, setCustomInput] = useState('')
  const customStyles = draft.filter((style) => !presetStyles.has(style))
  const toggle = (style: string) => setDraft((current) => current.includes(style) ? current.filter((item) => item !== style) : [...current, style])
  const addCustomStyle = () => {
    const style = customInput.trim()
    if (!style || draft.includes(style)) return
    setDraft((current) => [...current, style])
    setCustomInput('')
  }
  return <div className="modal-layer" onClick={onClose}><section className="profile-edit-sheet" onClick={(event) => event.stopPropagation()}><header className="filter-sheet-head"><div><p className="eyebrow">EDIT PREFERENCES</p><h2>{title}</h2></div><button className="modal-close inline-close" aria-label="关闭编辑" onClick={onClose}><X size={18} /></button></header><p className="profile-edit-copy">可跨分类多选。已选风格会作为搭配排序偏好，不同风格之间不互斥。</p>{styleGroups.map((group) => <section className="style-group" key={group.title}><div className="style-group-heading"><h3>{group.title}</h3><span>可多选</span></div><div className="style-choice-grid">{group.options.map((style) => <button key={style} className={draft.includes(style) ? 'selected' : ''} aria-pressed={draft.includes(style)} onClick={() => toggle(style)}>{style}</button>)}</div></section>)}<section className="style-group custom-style-group"><div className="style-group-heading"><h3>自定义风格</h3><span>例如：轻法式</span></div><div className="custom-style-input"><input value={customInput} onChange={(event) => setCustomInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); addCustomStyle() } }} placeholder="输入你喜欢的风格" maxLength={16} /><button onClick={addCustomStyle}>添加</button></div>{customStyles.length > 0 && <div className="style-choice-grid custom-style-list">{customStyles.map((style) => <button key={style} className="selected custom-style-chip" onClick={() => toggle(style)}>{style}<X size={12} /></button>)}</div>}</section><footer className="filter-sheet-actions"><button className="filter-reset" onClick={() => setDraft([])}>清空</button><button className="filter-apply" onClick={() => onSave(draft)}>保存偏好</button></footer></section></div>
}
...
function AccountSheet({ onClose, onSwitch, onLogout }: { onClose: () => void; onSwitch: () => void; onLogout: () => void }) {
  return <div className="modal-layer" onClick={onClose}>
    <section className="account-sheet" onClick={(event) => event.stopPropagation()}>
      <header className="account-head">
        <div className="account-avatar">林</div>
        <div><p className="eyebrow">CURRENT ACCOUNT</p><h2>林同学</h2><p>微信授权登录</p></div>
        <button className="modal-close inline-close" aria-label="关闭账户管理" onClick={onClose}><X size={18} /></button>
      </header>
      <div className="account-actions">
        <button onClick={onSwitch}><span className="account-action-icon"><UserRound size={18} /></span><span><strong>更换账号登录</strong><small>使用其他微信账号继续</small></span><ChevronRight size={17} /></button>
        <button className="logout-action" onClick={onLogout}><span className="account-action-icon"><LogOut size={18} /></span><span><strong>退出登录</strong><small>退出后可继续匿名浏览</small></span><ChevronRight size={17} /></button>
      </div>
    </section>
  </div>
}
...
function ItemEditSheet({ item, onClose, onSave }: { item: Item; onClose: () => void; onSave: (item: Item) => void }) {
  const [name, setName] = useState(item.name)
  const [category, setCategory] = useState<Exclude<Category, '全部'>>(item.category)
  const [color, setColor] = useState(item.color)
  const [fabric, setFabric] = useState(item.tags[0] || '')
  const [tags, setTags] = useState(item.tags.slice(1).join(', '))
  const [available, setAvailable] = useState(item.status === '可穿')

  const save = () => {
    const categoryVisual: Record<Exclude<Category, '全部'>, Item['visual']> = {
      上衣: 'shirt',
      裤子: 'pants',
      连衣裙: 'dress',
      半身裙: 'skirt',
    ...
    onSave({
      ...
      category,
      color: color.trim() || '未标注',
      tags: [fabric.trim(), ...tags.split(/[，,]/).map((tag) => tag.trim()).filter(Boolean)].filter(Boolean).slice(0, 4),
      status: available ? '可穿' : '暂时不能穿',
  ...
  return <div className="modal-layer" onClick={onClose}>
    <div className="confirm-sheet" onClick={(event) => event.stopPropagation()}>
      <div className="confirm-sheet-head">
        <div><p className="eyebrow">CLOTHING DETAILS</p><h2>编辑衣物信息</h2></div>
        <button className="modal-close inline-close" aria-label="关闭编辑" onClick={onClose}><X size={18} /></button>
      </div>
      <div className="recognition-layout edit-recognition-layout">
        <div className="upload-preview edit-item-preview"><ClothingVisual item={{ ...item, category, color, visual: item.visual }} /></div>
        <div className="recognition-status done"><Check size={15} />这是已确认的识别结果，可随时修改后保存。</div>
      </div>
      <div className="confidence-note"><Sparkles size={15} /><span>保存后的信息会立即同步到衣橱，并作为之后穿搭推荐的依据。</span></div>
      <div className="confirm-form">
        <label>衣物名称<input value={name} onChange={(event) => setName(event.target.value)} /></label>
        <label>衣物类别<select value={category} onChange={(event) => setCategory(event.target.value as Exclude<Category, '全部'>)}>{(['上衣', '裤子', '连衣裙', '半身裙', '外套', '鞋子', '配饰', '包袋'] as Exclude<Category, '全部'>[]).map((option) => <option key={option}>{option}</option>)}</select></label>
        <label>颜色<input value={color} onChange={(event) => setColor(event.target.value)} /></label>
        <label>面料/保暖标签<input value={fabric} onChange={(event) => setFabric(event.target.value)} /></label>
        <label className="wide-field">其他标签（逗号分隔）<input value={tags} onChange={(event) => setTags(event.target.value)} /></label>
        <button className={`availability-toggle ${available ? 'available' : ''}`} onClick={() => setAvailable(!available)}>{available ? <Check size={15} /> : <X size={15} />}{available ? '当前可穿，可参与搭配' : '暂时不能穿，不参与搭配'}</button>
      </div>
      <button className="confirm-add-btn" onClick={save}>保存衣物信息 <Check size={17} /></button>
    </div>
  </div>
}
